...
dev:
	@reflex -sr '\.go$$' -R '^web/' -R '^internal/server/ui' -R '^internal/worker/' -R '^config/' -- sh -c 'make server && ./build/abstruse-server'

dev_worker:
	@reflex -sr '\.go$$' -R '^web/' -R '^internal/server' -R '^config/' -- sh -c 'make worker && ./build/abstruse-worker'
