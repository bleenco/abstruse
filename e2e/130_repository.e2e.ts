...
describe('Repository', () => {
  ...
  it('should redirect to bterm repository, and update its repository provider', () => {
    return  browser.get('/repositories')
      ...
      .then((): any => isLoaded())
      .then((): any => browser.wait(() => element(by.css(`.selectbox-value`)).isPresent()))
      .then(() => element(by.css('.selectbox-value')).getText())
  ...
  it('should redirect to d3 repository, check build configuration file and run buld', () => {
    return  browser.get('/repositories')
      .then((): any => browser.wait(() => {
        return element.all(by.css('.list-item')).count().then(count => count === 2);
      }))
