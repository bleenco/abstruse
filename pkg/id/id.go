...
// Compare Compare the two given IDs.  Note that this function is NOT SAFE AGAINST
...
// Equals checks the two given IDs for equality.  This function uses a constant-time
// comparison algorithm to prevent timing attacks.
func (i ID) Equals(other ID) bool {
	return subtle.ConstantTimeCompare(i[:], other[:]) == 1
}

// MarshalText implements the `TextMarshaler` interface from the encoding package.
func (i *ID) MarshalText() ([]byte, error) {
	return []byte(i.String()), nil
}

// UnmarshalText implements the `TextUnmarshaler` interface from the encoding package.
