  ...
  resetForm(): void {
    this.form = {
      ...
      dockerfile: [
        ...
        'RUN set -xe \\',
        '    && apt update \\',
        '    && apt install -y --no-install-recommends ca-certificates curl build-essential libssl-dev git python xvfb x11vnc fluxbox xterm \\',
        '    && apt install -y --no-install-recommends openssh-server chromium-browser gconf-service gconf-service-backend gconf2-common \\',
        '    && apt install -y --no-install-recommends sudo libappindicator1 libdbus-glib-1-2',
        '',
        'RUN mkdir /var/run/sshd \\',
        '    && echo \'root:abstrusePass\' | chpasswd \\',
