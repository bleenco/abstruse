    ...
    if (!errors) {
      ...
      appSpecExists(container).then(exists => {
        ...
        .then(exists => {
          ...
          if (!exists) {
            const cmd = `aws deploy create-application --application-name ${preferences.bucket}`;
        ...
        .then(result => {
          ...
          const mes = style.yellow.open + style.bold.open + '==> deployment completed successfully!'
            + style.bold.close + style.yellow.close + '\r\n';
          observer.next({ type: 'data', data: mes });
          observer.complete();
        })
        .catch(err => {
          observer.error(err);
