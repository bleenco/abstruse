...
export function create(): Promise<null> {
  ...
  return new Promise((resolve, reject) => {
    schema.hasTable('users')
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('users', (t: knex.TableBuilder) => {
      ...
      .then(() => schema.hasTable('access_tokens'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('access_tokens', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
      ...
      .then(() => schema.hasTable('repositories'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('repositories', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
            t.integer('github_id');
      ...
      .then(() => schema.hasTable('builds'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('builds', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
            t.string('branch');
            t.integer('pr');
            t.integer('head_id');
      ...
      .then(() => schema.hasTable('build_runs'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('build_runs', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
            t.integer('head_id');
            t.dateTime('start_time');
            t.dateTime('end_time');
      ...
      .then(() => schema.hasTable('jobs'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('jobs', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
            t.text('data');
      ...
      .then(() => schema.hasTable('job_runs'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('job_runs', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
      ...
      .then(() => schema.hasTable('permissions'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('permissions', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
            t.integer('repositories_id').notNullable();
            t.foreign('repositories_id').references('repositories.id');
            t.integer('users_id').notNullable();
      ...
      .then(() => schema.hasTable('logs'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('logs', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
      ...
      .then(() => schema.hasTable('environment_variables'))
      .then(hasTable => {
        if (!hasTable) {
          return schema.createTable('environment_variables', (t: knex.TableBuilder) => {
            t.increments('id').unsigned().primary();
            t.integer('repositories_id').notNullable();
            t.foreign('repositories_id').references('repositories.id');
